...
$stream = ssh2_exec($connection, 'ubntbox mca-status | grep "ccq"');
while(!feof($stream)) {
 $ccq_live .= fread($stream, 1024);
 list($reciduo, $ccq_live2) = explode("=", $ccq_live);
}
$ccq_live3=($ccq_live2/10);
	 $sql_update=mysql_query("UPDATE INT_radio_planning SET ccq_live='$ccq_live3'  WHERE  ip='$ip' ");
}

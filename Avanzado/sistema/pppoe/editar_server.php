...
$sql_estacion=mysql_query("SELECT estacion FROM INT_radio_planning_estaciones WHERE  estacion='$estacion' ORDER BY estacion ASC ");
$sql_estacion2=mysql_query("SELECT estacion FROM INT_radio_planning_estaciones WHERE not estacion='$estacion' ORDER BY estacion ASC ");
$sql_editar=mysql_query("SELECT * FROM PPPoE_SRV WHERE  id='$id_server'");
...
<li><a href="../inventario/inventario.php">Inventario</a></li>
 ...
 <?php include '../../../include/estructura/submenu_clientes_busqueda_pppoe_ver_server.php';?>
  ...
  <div id="busqueda_clientes">
    <div id="datos_com"><strong>Editar Servidor PPPoE</strong>  <br />
      <br />
    </div>
    <FORM name=tuformulario autocomplete="off" ACTION="comprobacion_guardar_server.php" METHOD="POST">
  ...
  <tr>
    ...
    <input name="nombre_server" type="text" class="formulario_grande_intranet_editar" id="nombre_server" value="<?php echo $row['nombre_server'];?>"/>
    <input type="hidden" name="nombre_server2" value="<?php echo $row['nombre_server'];?>"/>
        <input type="hidden" name="ip_server2" value="<?php echo $row['ip_server'];?>"/>
  ...
  <tr>
    ...
    <td><select class="formulario_grande_intranet_editar" name="estacion"> 
	<?php while($row2 = mysql_fetch_array($sql_estacion)){ ?>
        <option value="<?php echo $row2['estacion']; ?>" ><?php echo $row2['estacion']; ?>
          <?php } ?>
          </option>
          <?php while($row3 = mysql_fetch_array($sql_estacion2)){ ?>
          <option value="<?php echo $row3['estacion']; ?>" ><?php echo $row3['estacion']; ?>
          <?php } ?>
          </option>
      </select>
       </td>
  ...
  <tr>
    ...
    <input name="ip_server" type="text" class="formulario_chico_intranet_editar" id="ip_server" value="<?php echo $row['ip_server'];?>"/>
  ...
  <tr>
    ...
    <input name="user_server" type="text" class="formulario_chico_intranet_editar" id="user_server" value="<?php echo $row['user_server'];?>" />
    <span class="textfieldRequiredMsg">Se necesita un valor.</span><span class="textfieldMinCharsMsg">No se cumple el mínimo de caracteres requerido.</span><span class="textfieldMaxCharsMsg">Se ha superado el número máximo de caracteres.</span></span></td>
  </tr>
  <tr>
    ...
    <input name="pass_server" type="text" class="formulario_chico_intranet_editar" id="pass_server" value="<?php echo $row['pass_server'];?>" />
  ...
  <tr>
    ...
    <input name="modelo" type="text" class="formulario_chico_intranet_editar" id="modelo" value="<?php echo $row['modelo'];?>"/>
    <span class="textfieldRequiredMsg">Se necesita un valor.</span><span class="textfieldMinCharsMsg">No se cumple el mínimo de caracteres requerido.</span></span></td>
  </tr>
  <tr>
  ...
  <tr>
    <td height="18" class="LINK"><input name="button" type="submit" class="boton_intranet" id="button" value="Guardar Registro" /></td>
    <td><span class="LINK">
      <input name="button2" type="reset" class="boton_intranet" id="button2" value="Cancelar" />
    </span></td>
  </tr>
  </table>
  <?php }?>
	...
	if ($id==2){
		...
		echo "<div id='dialog' title='Actualizar Registro'><p>Registro Actualizado Exitosamente</div>";

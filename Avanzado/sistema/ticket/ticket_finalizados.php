...
<?php include '../../../include/estructura/banner.php';?>
 ...
 <?php include '../../../include/estructura/submenu_ticket_finalizados.php';?>
	
     

  <div id="ticket_resultado3">
    <div id="titulo_ticket_abierto">
      <div id="titulo_ticket_abiertos_left">Revisión Terreno :</div>
      <div id="titulo_ticket_abiertos_right">
        <form action="ticket_finalizados.php" method="GET">
          <p>
...
SELECT * FROM TICKET WHERE status='Finalizado' AND NOT subtipo='Factibilidad' order by fecha_creacion DESC ");
...
echo "<td><div id='divi'><a   href='comprobacion_eliminar5.php?cliente=$row[1]&numero=$row[13]&ticket_filter=$ticket_filter'><center><img src='../../../imagenes/eliminar.png'</a></div></center></td> \n";

}
echo "</table></center> \n";
echo "<p>";

$sql = "SELECT COUNT(*)  FROM TICKET WHERE status='Finalizado' AND NOT subtipo='Factibilidad' order by fecha_creacion DESC ";
$resultado = mysql_query($sql);

$select=mysql_query("SELECT * FROM TICKET WHERE status='Finalizado' AND NOT subtipo='Factibilidad' order by fecha_creacion DESC ");
if (mysql_num_rows($select)>0){
 ...
 echo "Cantidad de Registros = ".$row[0]; }
	...
	echo "No tienes Ticket  que coincidan con el Filtro de Busqueda.";			
	  ...
	  $sql_abiertos2=mysql_query("SELECT * FROM TICKET WHERE status='Finalizado' AND subtipo='Factibilidad' order by fecha_creacion DESC ");
...
$sql = "SELECT COUNT(*)  FROM TICKET WHERE status='Finalizado'  AND subtipo='Factibilidad' order by fecha_creacion DESC ";
$resultado = mysql_query($sql);

$select=mysql_query("SELECT * FROM TICKET WHERE status='Finalizado'   AND subtipo='Factibilidad' order by fecha_creacion DESC ");
if (mysql_num_rows($select)>0){
 $row = mysql_fetch_array($resultado);
 echo "Cantidad de Registros = ".$row[0]; }
else {echo "";}
	...
	echo "No tienes Ticket  que coincidan con el Filtro de Busqueda.";			
	  ...
	  $sql_abiertos2=mysql_query("SELECT * FROM TICKET WHERE status='Finalizado'   order by fecha_creacion DESC ");
...
$sql = "SELECT COUNT(*) FROM TICKET WHERE status='Finalizado'  order by fecha_creacion DESC ";
$resultado = mysql_query($sql);

$select=mysql_query("SELECT * FROM TICKET WHERE status='Finalizado'   order by fecha_creacion DESC ");
if (mysql_num_rows($select)>0){
 $row = mysql_fetch_array($resultado);
 echo "Cantidad de Registros = ".$row[0]; }
else {echo "";}
...
?></p>
   ...
   if ($id==1){
	   ...
	   <center><form method='GET' action='comprobacion_eliminado5.php'>

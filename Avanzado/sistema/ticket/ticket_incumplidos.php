...
<?php include '../../../include/estructura/banner.php';?>
 ...
 <?php include '../../../include/estructura/submenu_ticket_incumplidos.php';?>
...
echo "<td><div id='divi'><a   href='comprobacion_eliminar1.php?cliente=$row[1]&numero=$row[13]&ticket_filter=$ticket_filter'><center><img src='../../../imagenes/eliminar.png'</a></div></center></td> \n";

}
echo "</table></center> \n";
echo "<p>";

$sql = "SELECT COUNT(*) FROM TICKET  WHERE status='Abierto' and    fecha_solucion<'$fecha2'  AND NOT subtipo='Factibilidad' order by fecha_creacion DESC ";
$resultado = mysql_query($sql);

$select=mysql_query("SELECT * FROM TICKET  WHERE status='Abierto' and    fecha_solucion<'$fecha2'  AND NOT subtipo='Factibilidad' order by fecha_creacion DESC");
if (mysql_num_rows($select)>0){
 ...
 echo "Cantidad de Registros = ".$row[0]; }
  ...
  $fecha2=date("Y-m-d H:i:s");
$sql_abiertos2=mysql_query("SELECT * FROM TICKET  WHERE status='Abierto' and    fecha_solucion<'$fecha2'  AND  subtipo='Factibilidad' order by fecha_creacion DESC  ");
...
$sql = "SELECT COUNT(*) FROM TICKET  WHERE status='Abierto' and    fecha_solucion<'$fecha2'  AND  subtipo='Factibilidad' order by fecha_creacion DESC ";
$resultado = mysql_query($sql);

$select=mysql_query("SELECT * FROM TICKET  WHERE status='Abierto' and    fecha_solucion<'$fecha2'  AND  subtipo='Factibilidad' order by fecha_creacion DESC ");
if (mysql_num_rows($select)>0){
 $row = mysql_fetch_array($resultado);
 echo "Cantidad de Registros = ".$row[0]; }
else {echo "";}
	...
	echo "No tienes Ticket  que coincidan con el Filtro de Busqueda.";			
	  	  ...
	  	  $fecha2=date("Y-m-d H:i:s");
$sql_abiertos2=mysql_query("SELECT * FROM TICKET  WHERE status='Abierto' and    fecha_solucion<'$fecha2' order by fecha_creacion ");
...
$sql = "SELECT COUNT(*) FROM TICKET  WHERE status='Abierto' and    fecha_solucion<'$fecha2' order by fecha_creacion ";
$resultado = mysql_query($sql);

$select=mysql_query("SELECT * FROM TICKET  WHERE status='Abierto' and    fecha_solucion<'$fecha2' order by fecha_creacion ");
if (mysql_num_rows($select)>0){
 $row = mysql_fetch_array($resultado);
 echo "Cantidad de Registros = ".$row[0]; }
else {echo "";}
   ...
   if ($id==1){
	   ...
	   <center><form method='GET' action='comprobacion_eliminado2.php'>

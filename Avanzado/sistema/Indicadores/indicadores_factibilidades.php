...
<?php include '../../../include/estructura/banner.php';?>
 ...
 <?php include '../../../include/estructura/submenu_indicadores_factibilidades.php';?>
  ...
  <div id="kpi_front">
    ...
    <?php if (empty($id_filter)){?><p><strong>KPI Factibilidades</strong>        
    <p>Seleccione el Mes y el Año :    </p>
    <form id="form1" name="form1" method="post" action="comprobacion_kpi_factibilidad.php">
      <table width="772" border="0">
      <tr>
        <td width="52">Mes </td>

        <td width="181"><select name="mes" class="formulariochico" id="mes">
        ...
        </select><td width="22">&nbsp;</td>
        <td width="49">Año</td>
        <td width="202"><select name="anios" class="formulariochicamoficicar">
<?php while($row = mysql_fetch_array($sql_anios)){ ?>
      <option value='<?php echo $row['anios'];?>'><?php echo $row['anios']; ?></option><?php } ?>
                
      </select></td>
        <td width="240"><input name="btn_submit" type="submit" class="formulariochico" value="Buscar" />&nbsp;</td>
      </tr>
    </table>
      <br />
      <br />
    ...
    <p>
	  ...
	  $id_fac=$_GET["id_fac"];
	  if ($id_fac==1){
		  	  echo "<div id='dialog' title='Busqueda'><p>No registra factibiliades en el período seleccionado. Intente con otra Fecha.</div>";
